class PostsController < ApplicationController
  ...
  def create
    ...
    if the_post.valid?
      the_post.save
      redirect_to("/boards/" + the_post.board_id.to_s, { :notice => "Post created successfully." })
    else
      redirect_to("/boards/" + the_post.board_id.to_s, { :alert => the_post.errors.full_messages.to_sentence })
    end
  end
